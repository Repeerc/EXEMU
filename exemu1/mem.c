...
Boolean memAccess(ArmMem* mem, UInt32 addr, UInt8 size, Boolean write, void* buf){
	...
	for(i = 0; i < MAX_MEM_REGIONS; i++){
		//printf("addr:%x, regions:%d, pa:%x, sz:%x, range:%lx \r\n",addr, i, mem->regions[i].pa, mem->regions[i].sz, (mem->regions[i].pa + mem->regions[i].sz));
		if( (mem->regions[i].pa <= addr) && (addr < (mem->regions[i].pa + mem->regions[i].sz)))
		{
